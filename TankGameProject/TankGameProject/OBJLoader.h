...
//                int v = std::atoi(vStr.c_str());
//                int vt = std::atoi(vtStr.c_str());
//                int vn = std::atoi(vnStr.c_str());
...
//                glm::vec3 faceNormal = glm::normalize(cross(U, V));
//
//                for (size_t j = 0; j < 3; ++j)
//                {
//                    Vertex vert;
//                    vert.position = glm::vec3(positions[p[j]->v]);
//                    vert.texUV = glm::vec2(texcoords[p[j]->vt]);
...
//        glTexCoordPointer(2, GL_FLOAT, sizeof(Vertex), &model[0].texUV);
...
//}
//
//std::vector<Texture> loadMaterialTextures(string typeName)
//{
...
namespace objl
{
	...
	public:
		...
		void display(GLFWwindow* window, float width, float height)
		{
			glClearColor(0.2f, 0.2f, 0.2f, 1.0f);
			glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
			...
			glPushMatrix();
			{
				...
				glEnableClientState(GL_TEXTURE_COORD_ARRAY);
				glEnableClientState(GL_NORMAL_ARRAY);
				glVertexPointer(3, GL_FLOAT, sizeof(Vertex), &LoadedMeshes[0].Indices);
				glTexCoordPointer(2, GL_FLOAT, sizeof(Vertex), &LoadedMeshes[0].Vertices);
				glNormalPointer(GL_FLOAT, sizeof(Vertex), &LoadedMeshes[0].Vertices);
				glDrawArrays(GL_TRIANGLES, 0, LoadedMeshes.size());
				glDisableClientState(GL_VERTEX_ARRAY);
				glDisableClientState(GL_TEXTURE_COORD_ARRAY);
				glDisableClientState(GL_NORMAL_ARRAY);
