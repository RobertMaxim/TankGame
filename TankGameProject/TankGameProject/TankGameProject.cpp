...
//bool res1 = loadOBJ("14077_WWII_Tank_Germany_Panzer_III_v1_L2.obj", vertices, uvs, normals);
//

std::vector< glm::vec3 > vertices1;
std::vector< glm::vec2 > uvs1;
std::vector< glm::vec3 > normals1; // Won't be used at the moment.
bool res2 = loadOBJ("430.obj", vertices1, uvs1, normals1);
...
void renderScene(const Shader& shader)
{
	...
	model = glm::mat4();
	model = glm::translate(model, glm::vec3(5.0f, -0.5f, 25.0f));
	///model = glm::scale(model, glm::vec3(0.75f));
	shader.SetMat4("model", model);
	renderCube();
}
